...
SELECT `temp2`.`pt_no`
    ...
    , `temp2`.`rx_ara_c_dose`
    , `temp2`.`rx_apl_regimen(atra)`
    , `temp2`.`rx_apl_regimen(ato)`
    , `temp2`.`rx_apl_regimen(arac)`
    , `temp2`.`rx_apl_regimen(dauno)`
    , `temp2`.`rx_apl_regimen(ida)`
    , `temp2`.`rx_apl_regimen(ot)`
    , `temp2`.`rx_hma`
...
SELECT 
    ...
    Temp2.rx_ara_c_dose,
    concat(
        'Backbone: ',`(rx_ara_c_dose_text)`,char(13),
        'APL: ',`(rx_apl_regimen_text)`,char(13),
        'Growth factor: ',`(rx_gf_text)`,char(13),
        'Antibodies: ',`(rx_antibodies_text)`,char(13),
        'Inhibitors: ',`(rx_inhibitors_text)`,char(13),
        'Non-Chemo:',`(rx_ot_nc_text)`,char(13),
        'Chemo:',`(rx_ot_text)`
    ) AS All_addon,
...
SELECT * FROM Temp.Temp2 ORDER BY pt_no;

SELECT * FROM Temp.Temp2 where pt_no < 0;

SELECT * FROM Temp.Temp2 where pt_no > 0;

SET @idno := 0;

SELECT @idno:=@idno+1 AS sequence, pt_no-5499, pt_no FROM temp.temp2 where pt_no > 0 ;

SELECT * FROM Temp.temp2
WHERE pt_no IN (
    SELECT pt_no FROM Temp.Temp2
        WHERE pt_no > 0
        GROUP BY pt_no , PtMRN , rx_date
        HAVING COUNT(pt_no) > 1 )
ORDER BY pt_no, PtMRN;
...
SELECT BackBoneName, BackBoneAddOn, CalculatedAddOn, MedTxAgent, count(*)  FROM temp.temp2 where rx_backbone = 32 group by 1,2,3,4;
