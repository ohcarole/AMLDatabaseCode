...
def create_temp_mutation_table(cnxdict, labtest, writer):
    ...
    cmd = 'SELECT * FROM temp.{0};'.format(viewshortname)
    df = dosqlread(cmd, cnxdict['cnx'])
    df.to_excel(writer, sheet_name=viewshortname, index=False)
    return


def create_arrival_mutation_summary(cnxdict, cmd, joincmd, writer):
    cnxdict['sql'] = """
...
def MainRoutine(cnxdict):
    ...
    if keepongoing == 'yes':
        ...
        for labtest in lablist:
            ...
            create_temp_mutation_table(cnxdict, labtest, writer)
        ...
        for timepoint in timepointlist:
            for labtest in lablist:
                ...
                """.format('tbl_' + str(tblnum), timepoint, labtest.replace('v_',''))
        ...
        create_arrival_mutation_summary(cnxdict, cmd, joincmd, writer)
