...
def get_caisis_tables(cnxdict, engine):
    tbllist = """
        vDatasetEncounters
...
def get_pathology(cnxdict,engine):
    tbl = 'vDatasetPathology'
    key = 'PatientId'
    tempsql = """
        SELECT * FROM [WorkDBProd]..[vDatasetPathology];
    """
...
def get_pathtest(cnxdict,engine):
    tbl='vDatasetPathTest'
    key='PathTestId'
    tempsql = """
            SELECT * FROM [WorkDBProd]..[vDatasetPathTest];
    	"""
...
def get_labtest(cnxdict,engine):
    tbl='vDatasetLabTests'
    tempsql = """
            SELECT * FROM [WorkDBProd]..[vDatasetLabTests];
    	"""
    print('-- Get SQL Server laboratory test data')
    df = dosqlread(tempsql, cnxdict)
    # Add a test here to see if we need to keep going, if 0 records QUIT!  :)
    print('-- Store SQL Server {} data'.format(tbl))
    putinmysql(df, tempsql, tbl, engine)
