...
/*
Remove records where the patient was not arriving for AML
*/
...
# More than one treatment potentially relevant for these treatments
...
DROP TABLE IF EXISTS temp.treatmentfound ;
CREATE TABLE temp.treatmentfound 
    SELECT PtMRN, ArrivalDate, count(*) as `reccount` 
    FROM temp.Result2 WHERE FirstTreatment = 'Is First' GROUP BY PtMRN, ArrivalDate ;
SELECT * FROM temp.treatmentfound WHERE `reccount` > 1;
SELECT * FROM temp.result2 a join temp.treatmentfound b on a.ptmrn = b.ptmrn and a.arrivaldate = b.arrivaldate 
    where b.`reccount` > 1 ;

SELECT * FROM caisis.vdatasetarrivalwithprevnext;
SELECT * FROM temp.Result1 ;
SELECT * FROM temp.Result2 ;
SELECT * FROM temp.Result2 WHERE FirstTreatment = 'Is First';
...
CREATE TABLE temp.AllArrivals
    SELECT a.PtMRN
        ...
        , NextArrivalDate
        FROM Temp.Result2 a
        JOIN (SELECT PtMRN, PtLastName, PtBirthdate, PtDeathDate, PtDeathType FROM caisis.vdatasetpatients) b on a.PtMRN = b.PtMRN ;
